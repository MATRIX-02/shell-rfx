...
const CategoryCard = () => {
	...
	const handleCategorySelect = async (category) => {
		...
		try {
			const [categoryCardDataResponse] = await Promise.all([
				axios.get(
					`${MASTER_DATA}/categoryCard/categoryCardData/${category?.category_card_id}`
				),
			]);
	...
	const handleAddCategories = async (newCategories) => {
		const categoriesWithId = newCategories.map((category) => {
			const formattedSubcategoryName = category.subcategory_name.replace(
				/\s+/g,
				"_"
			);
			return {
		...
		categoriesWithId.forEach((category) => {
			toast.success(
				`Category Card ID is created: ${category.category_card_id}`
			);
		});
		...
		try {
			for (const category of categoriesWithId) {
				...
				const apiCategory = {
					...
					parent_category_name: category.parent_category,
					description: category.parent_category_description,
					category_level: parseInt(category.category_level),
					unspsc_code: category.unspsc_code,
					sub_categories: category.categories.map((sub) => ({
						id: sub.category_id,
						name: sub.category,
		...
		try {
			...
			setCategories((prevCategories) =>
				prevCategories.map((cat) =>
					cat.category_card_id === categoryCardId
			...
			if (
				selectedCategory &&
				selectedCategory.category_card_id === categoryCardId
			) {
				setSelectedCategory((prev) => ({ ...prev, status: newStatus }));
			}

			await axios.patch(
				`${MASTER_DATA}/categoryCard/toggleCategoryCardActivation/${categoryCardId}?status=${newStatus}`
			);
